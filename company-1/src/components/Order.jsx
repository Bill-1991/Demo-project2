...
function Order({
  ...
  return chosenItems.length && order === true ? (
    <Card id="order" className="order">
      <button className="close" onClick={handleOrder}>
        <p>X</p>
      </button>
      ...
      <div className="tables">
        ...
        <div className="allTables">
          {tables.map((table) => (
            <button key={table} onClick={() => handleCurTable(table)}>
              {table}
            </button>
          ))}
        </div>
        <button className="send" onClick={() => handleSaveFile(company)}>
          Send Order
        </button>
      </div>
    </Card>
